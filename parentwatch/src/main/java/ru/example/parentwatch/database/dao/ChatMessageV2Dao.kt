...
import ru.example.parentwatch.database.entity.ChatMessageV2Entity
...
interface ChatMessageV2Dao {
    ...
    suspend fun getPageNewestFirst(conversationId: String, limit: Int, offset: Int): List<ChatMessageV2Entity>
    ...
    fun observeLatest(conversationId: String, limit: Int): Flow<List<ChatMessageV2Entity>>
    ...
    @Query(
        "UPDATE chat_messages_v2 SET status = :status, delivery_state = :deliveryState, " +
            ...
            "read_at = :readAt, sync_state = :syncState WHERE message_id = :messageId"
    )
