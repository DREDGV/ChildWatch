...
- ParentWatch app: `parentwatch/src/main/java/ru/example/parentwatch/` — родительское приложение для мониторинга.
...
- **Live preview:** scrcpy установлен и настроен для real-time отображения устройств.
- **Dual device workflow:** ParentWatch → Nokia G21 (реальный), ChildWatch → Pixel 8 Emulator (обход Family Link).
- **Quick commands:** `.\scripts\dev-workflow.ps1 -Action setup|deploy|test|cleanup`
- **VS Code tasks:** 14 задач для автоматизации (см. `.vscode/tasks.json`)
- **Documentation:** `.vscode/README.md` — навигация по всей dev документации.
