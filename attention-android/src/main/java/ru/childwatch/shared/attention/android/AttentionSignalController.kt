...
class AttentionSignalController(
    ...
    interface Callback {
        ...
        fun onFailed(request: AttentionSignalRequest, code: String, error: Throwable?)
    }
    ...
    fun start(raw: AttentionSignalRequest) {
        ...
        request.validationError()?.let { code ->
            callback.onFailed(request, code, null)
            return
        }

        active?.let { stopInternal("REPLACED", notify = true) }
        stopping.set(false)
        active = request
            ...
            timeout = Runnable {
                synchronized(this) {
                    if (active?.requestId == request.requestId) {
                        stopInternal("COMPLETED", notify = false)
                        callback.onCompleted(request)
                    }
                }
            }.also { handler.postDelayed(it, request.durationMs) }
            callback.onStarted(request)
        } catch (error: Throwable) {
            cleanup()
            active = null
            callback.onFailed(request, "PLAYBACK_FAILED", error)
        }
    }

    @Synchronized
    fun stop(requestId: String? = null, reason: String = "LOCAL_USER"): Boolean {
        val current = active ?: return false
        if (!requestId.isNullOrBlank() && current.requestId != requestId) return false
    ...
    private fun startSound(request: AttentionSignalRequest) {
        ...
        val uri = resolveUri(request.tone) ?: error("No built-in alarm or ringtone is available")
    ...
    private fun resolveUri(tone: AttentionTone): Uri? {
        val type = when (tone) {
            AttentionTone.ATTENTION -> RingtoneManager.TYPE_NOTIFICATION
            AttentionTone.RINGTONE -> RingtoneManager.TYPE_RINGTONE
            AttentionTone.ALARM, AttentionTone.SIREN -> RingtoneManager.TYPE_ALARM
        }
        return RingtoneManager.getActualDefaultRingtoneUri(appContext, type)
    ...
    private fun setTemporaryAlarmVolume(percent: Int) {
        ...
        val value = (max * percent.coerceIn(0, 100) / 100f).toInt().coerceIn(0, max)
    ...
    private fun restoreAlarmVolume() {
        ...
        runCatching {
            ...
            audioManager.setStreamVolume(AudioManager.STREAM_ALARM, old.coerceIn(0, max), 0)
        }
    }

    private fun startVibration(request: AttentionSignalRequest) {
        if (!request.vibrate || request.vibrationPattern == AttentionVibrationPattern.OFF) return
        ...
        val pattern = when (request.vibrationPattern) {
            AttentionVibrationPattern.OFF -> longArrayOf(0)
            AttentionVibrationPattern.PULSE -> longArrayOf(0, 500, 500)
            AttentionVibrationPattern.URGENT -> longArrayOf(0, 300, 150, 300, 150, 700)
            AttentionVibrationPattern.SOS -> longArrayOf(
                0, 200, 150, 200, 150, 200, 350,
    ...
    private fun acquireWakeLock(durationMs: Long) {
        ...
        wakeLock = powerManager.newWakeLock(
            ...
            acquire((durationMs + 10_000L).coerceAtMost(90_000L))
        }
    }

    private fun stopInternal(reason: String, notify: Boolean) {
        ...
        active = null
        stopping.set(false)
        if (notify) callback.onStopped(request, reason)
    }
